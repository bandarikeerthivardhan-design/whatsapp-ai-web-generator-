...
const App = () => {
  const content = {"about_text": "At Slice \u0026 Stone, we believe pizza is an art form. Founded in 2010, we have been serving our community with the finest wood-fired pizzas, crafted with passion and the freshest ingredients. From our family to yours, enjoy a slice of perfection.", "cta_text": "Order Online Now", "feature_descriptions": ["Hand-Tossed Dough: Made fresh daily using traditional Italian recipes.", "Fresh Ingredients: Locally sourced vegetables and premium meats.", "Wood-Fired Oven: Baked to perfection at 800 degrees for that perfect smoky crust."], "footer_text": "\u00a9 2024 Slice \u0026 Stone Pizzeria. All rights reserved. Made with love and extra cheese.", "hero_description": "Experience the authentic taste of stone-baked pizzas made with fresh, locally-sourced ingredients and hand-tossed dough.", "hero_heading": "Hot, Fresh, and Delivered Straight to Your Door"};
  const features = ["Online Ordering", "Interactive Menu Gallery", "Table Reservation", "Customer Reviews", "Google Maps Integration"];
  const theme = "vibrant";
    ...
    <div className={`min-h-screen ${isDark ? 'bg-slate-900 text-white' : 'bg-white text-gray-900'}`}>
      ...
      <nav className="p-6 flex justify-between items-center border-b border-gray-700/50">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-2xl font-bold tracking-tight text-primary">Slice & Stone Pizzeria</h1>
        </div>
        <div className="space-x-6 hidden md:flex">
          ...
          <a href="#" className="hover:text-primary transition-colors capitalize">Order Online</a>
      ...
      <section className="py-20 px-6 max-w-4xl mx-auto text-center">
        <h3 className="text-3xl font-bold mb-6">About Slice & Stone Pizzeria</h3>
      ...
      <footer className={`py-12 px-6 border-t ${isDark ? 'border-gray-800' : 'border-gray-200'}`}>
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-center gap-8">
          <div>
            <h4 className="text-xl font-bold text-primary mb-2">Slice & Stone Pizzeria</h4>
            <p className="opacity-60 max-w-xs">{content.footer_text}</p>
          </div>
          <div className="flex flex-col items-center md:items-end">
            <p className="font-medium">Contact Us</p>
            <p className="opacity-70">info@sliceandstone.com</p>
        ...
        <div className="mt-12 pt-8 border-t border-gray-800/10 text-center opacity-40 text-sm">
          &copy; {new Date().getFullYear()} Slice & Stone Pizzeria. All rights reserved.
        </div>
      </footer>
    </div>
  );
